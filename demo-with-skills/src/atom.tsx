...
/** Mapper that drops cva-only variant keys so they don't leak onto the native DOM element. */
export const omitVariants = <P extends object>(...keys: (keyof never)[]) =>
	(props: Partial<P> & Record<PropertyKey, unknown>): P => {
		const rest = { ...props };
		for (const key of keys) delete rest[key as never];
		return rest as P;
	};
